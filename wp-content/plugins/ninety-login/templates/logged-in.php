...
<div class="td-welcome-login"><p><?php _e('Welcome','gameleon'); ?> <?php echo $current_user->display_name; ?>!</p></div>

<?php if ( defined( 'MYARCADE_VERSION') ) : ?>

<?php
if ($last_login = (int) get_user_meta($current_user->ID, 'nd_login_time', true)) echo '<p>'.__('You last logged in on ','gameleon').date_i18n(__('l \t\h\e jS \of F, Y \a\t h:ia','gameleon'), $last_login).'.</p>';
...
echo '<p>';
echo _n('Since last logging in there has been ', 'Since last logging in there have been ', $posts, 'gameleon');
echo '<span class="count">'.$posts.'</span>';
echo _n('new game and ', 'new games and ', $posts, 'gameleon');
echo '<span class="count">'.$comments.'</span>';
echo _n('new comment', 'new comments', $comments, 'gameleon');
echo '</p>';
?>
...
echo _n('Since last logging in there has been ', 'Since last logging in there have been ', $posts, 'gameleon');
echo '<span class="count">'.$posts.'</span>';
echo _n('new post and ', 'new posts and ', $posts, 'gameleon');
echo '<span class="count">'.$comments.'</span>';
echo _n('new comment', 'new comments', $comments, 'gameleon');
echo '</p>';
?>
...
<li><a href="<?php echo site_url('/wp-admin/'); ?>"><?php _e( 'Dashboard', 'gameleon' ); ?></a></li>
<?php endif; ?>


<?php if( function_exists( 'bp_is_active' ) ): ?>
<li><a href="<?php bp_loggedin_user_link() ?>settings/"><?php _e( 'Edit Profile', 'gameleon' ); ?></a></li>
<?php else: ?>
<li><a href="<?php echo site_url('/wp-admin/profile.php'); ?>"><?php _e( 'Edit Profile', 'gameleon' ); ?></a></li>
<?php endif; ?>

<li><a href="<?php echo wp_logout_url( $current_url ); ?>"><?php _e( 'Log out', 'gameleon' ); ?></a></li>
</ul>
</div>
</div>
...
<?php if ( defined( 'MYARCADE_VERSION') ): ?>
<div class="td-welcome-minitabs"><?php _e('Recently played games:','gameleon'); ?></div>
<?php else: ?>
<div class="td-welcome-minitabs"><?php _e('Recently viewed articles:','gameleon'); ?></div>
<?php endif; ?>
...
_e('You have not played any games recently.', 'gameleon' );
else :
	_e('You have not viewed any articles recently.', 'gameleon' );
endif;
...
<div class="td-welcome-minitabs-2"><?php _e( 'My Recent Comments:','gameleon' ); ?></div>
<?php
global $wpdb;
...
echo '<p>';
_e('You have not made any comments.', 'gameleon');
echo '</p>';
endif;
?>
...
<?php _e( 'Users browsing this page:', 'gameleon' ); ?>
</div>
...
<?php _e( 'Most users ever seen online:', 'gameleon' ); ?>
</div>

<div class="online-users-content-2">
<?php echo get_most_users_online(); ?> <?php _e( 'on', 'gameleon' ); ?> <?php echo get_most_users_online_date(); ?>
</div>
<?php endif; ?>
