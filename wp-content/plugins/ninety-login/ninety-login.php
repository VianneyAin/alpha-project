<?php
/*
Plugin Name: Ajax Login/Register Box
...
class Ninety_Login extends Ninety_Base {
	...
	function __construct() {
		...
		// Localise
		load_plugin_textdomain( 'gameleon', false, dirname( plugin_basename( __FILE__ ) ).'/languages' );

		if ( ! is_admin() ) {
			add_action( 'wp_enqueue_scripts', array( &$this, 'scripts' ) );
			// add_action( 'init', array( &$this, 'styles' ) ); // styles added into theme's css
	 ...
	 */
	function scripts() {
		wp_register_script( 'blockui', $this->plugin_url() . '/js/blockui.js' , 'jquery', '1.1.2', true );
		wp_enqueue_script( 'ajax_login_js', $this->plugin_url() . '/js/login.js' , array( 'jquery', 'blockui' ), '1.1.2', true );
	}
