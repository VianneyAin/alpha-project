...
/*----------------------------------------------------------------------------------------------------------
	ADD THEME'S BREADCRUMB STYLE FOR THE REQUIRED WOOCOMMERCE PAGES.
-----------------------------------------------------------------------------------------------------------*/

function gameleon_woo_breadcrumbs( $defaults ) {
...
add_filter( 'woocommerce_breadcrumb_defaults', 'gameleon_woo_breadcrumbs', 10, 1 );


/*----------------------------------------------------------------------------------------------------------
	REMOVE THE bbPress BREADCRUMBS AS THEY ARE DUPLICATE TO THE THEMES BREADCRUMBS.
-----------------------------------------------------------------------------------------------------------*/

function gameleon_remove_bbp_breadcrumbs( $trail, $crumbs, $r ) {
...
add_filter( 'bbp_get_breadcrumb', 'gameleon_remove_bbp_breadcrumbs', 10, 3 );


/*----------------------------------------------------------------------------------------------------------
	BREADCRUMB LISTS COMPATIBILITY WITH FEW PLUGINS. LOAD THE BREADCRUMB FUNCTIONS FROM THE ACTIVE PLUGINS.
-----------------------------------------------------------------------------------------------------------*/

function get_gameleon_breadcrumb_lists() {
	global $smof_data;
	if ( 0 == $smof_data['td_breadcrumb'] ) {
		return;
		...
		gameleon_breadcrumb_lists();
	}
}


/*----------------------------------------------------------------------------------------------------------
	BREADCRUMB LISTS - ALLOWS VISITORS TO QUICKLY NAVIGATE BACK TO A PREVIOUS SECTION OR THE ROOT PAGE.
-----------------------------------------------------------------------------------------------------------*/

if ( !function_exists( 'gameleon_breadcrumb_lists' ) ) {

	function gameleon_breadcrumb_lists() {

		/* === OPTIONS === */

		$text['home']     = __( 'Home', 'gameleon' );								// text for the 'Home' link
		$text['category'] = __( 'Archive for %s', 'gameleon' );						// text for a category page
		$text['search']   = __( 'Search results for: %s', 'gameleon' ); 			// text for a search results page
		$text['tag']      = __( 'Posts tagged %s', 'gameleon' ); 					// text for a tag page
		$text['author']   = __( 'View all posts by %s', 'gameleon' ); 				// text for an author page
		$text['404']      = __( 'Error 404', 'gameleon' ); 							// text for the 404 page

																					// TODO make these as theme options
		$show['current'] = 1;														// 1 - show current post/page title in breadcrumbs, 0 - don't show
		$show['home']    = 0;														// 1 - show breadcrumbs on the home page, 0 - don't show

		$delimiter = ' <span class="td-breadcrumb-arrow">&#xf105;</span> ';			// delimiter between crumbs
		...
		if ( is_front_page() ) {
			...
			if ( get_query_var( 'paged' ) || get_query_var( 'page' ) ) {
				...
				$html_output .= $delimiter . sprintf( __( 'Page %s', 'gameleon' ), $page_num );
	...
	}	// END function gameleon_breadcrumb_lists()
