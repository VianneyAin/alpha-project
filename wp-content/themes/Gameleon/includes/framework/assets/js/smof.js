...
jQuery(document).ready(function($){
	...
	//(un)fold options in a checkbox-group
  	jQuery('.fld').click(function() {
    	var $fold='.f_'+this.id;
    	$($fold).slideToggle(); // $($fold).slideToggle('slow', "swing");
  	});
	...
	$.fn.center = function () {
		this.animate({"top":( $(window).height() - this.height() - 0 ) / 2+$(window).scrollTop() + "px"},100);
		this.css("left", 390 );
		return this;
	}
	...
	$('#of_backup_button').live('click', function(){
		...
		$('.panel_opacity_saving').css('display', 'block');
	...
	$('#of_save').live('click',function() {
		...
		$.post(ajaxurl, data, function(response) {
			...
			window.setTimeout(function(){
			//hide opacity over the panel
			$('.panel_opacity_saving').css('display', 'none');
				success.fadeOut();
				fail.fadeOut();
			}, 2000);
		});
	...
	$('#of_reset').click(function() {
		$('.panel_opacity_saving').css('display', 'block');
		//confirm reset
		var answer = confirm("Click OK to reset. All theme settings will be lost and replaced with default settings!");
	...
	jQuery(".cb-enable").click(function(){
		...
		jQuery($fold).slideDown('fast', "swing");
	});
	jQuery(".cb-disable").click(function(){
		...
		var obj = jQuery(this);
		var $fold='.f_'+obj.data('id');
		jQuery($fold).slideUp('fast', "swing");
	});
