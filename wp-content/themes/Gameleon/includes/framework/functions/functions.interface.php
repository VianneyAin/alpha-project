...
function optionsframework_add_admin() {
	...
	$of_page = add_menu_page( 'Theme Options Panel', 'Theme Options', 'edit_theme_options', 'gameleon_panel', 'optionsframework_options_page');

	// Add framework functionaily to the head individually
	add_action("admin_print_scripts-$of_page", 'of_load_only');
	add_action("admin_print_styles-$of_page",'of_style_only');
	add_action("admin_print_styles", 'td_composer_visual_style');
 ...
 * @since 1.0.0
 */
function td_composer_visual_style() {
	$td_minified_css = true; // for developer only: set to "false" to make minor changes
	if ( true == $td_minified_css ) {
		wp_enqueue_style('td_composer_visual_style', ADMIN_DIR . 'assets/css/vc-composer-style.min.css');
	}
	else {
		wp_enqueue_style('td_composer_visual_style', ADMIN_DIR . 'assets/css/vc-composer-style.css');
	}
}

function of_style_only() {
	// for developer only: set to "false" to make minor changes to admin-style.css and jquery-ui-custom.css then compile both in one file(admin-style.min.css) and minify.
	$td_minified_css = true;
	if ( true == $td_minified_css ) {
		wp_enqueue_style('admin-style-min', ADMIN_DIR . 'assets/css/admin-style.min.css');
	}
	else {
	...
	if( is_rtl() ) {
		wp_enqueue_style('rtl-admin-style', ADMIN_DIR . 'assets/css/rtl.min.css');
	}
...
function of_load_only() {
    ...
    // Enqueue Ace Editor scripts - http://ace.c9.io/
	wp_enqueue_script( 'ace_code_highlighter_js', ADMIN_DIR . 'assets/ace/ace.js', '', '1.0.0', true );
	wp_enqueue_script( 'ace_css_js', ADMIN_DIR . 'assets/js/ace-css.js', array( 'jquery', 'ace_code_highlighter_js' ), '1.0.0', true );
