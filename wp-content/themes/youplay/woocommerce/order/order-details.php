...
?>
<h2><?php _e( 'Order Details', 'youplay' ); ?></h2>
<table class="table table-bordered">
	<thead>
		<tr>
			<th class="product-name"><?php _e( 'Product', 'youplay' ); ?></th>
			<th class="product-total"><?php _e( 'Total', 'youplay' ); ?></th>
		</tr>
	</thead>
	<tbody>
		<?php
							...
							<?php
								...
								if ( $_product && $_product->exists() && $_product->is_downloadable() && $order->is_download_permitted() ) {
									...
									foreach ( $download_files as $download_id => $file ) {
										...
										$links[] = '<small><a href="' . esc_url( $file['download_url'] ) . '">' . sprintf( __( 'Download file%s', 'youplay' ), ( count( $download_files ) > 1 ? ' ' . $i . ': ' : ': ' ) ) . esc_html( $file['name'] ) . '</a></small>';
									}
	...
	<?php
				...
				if ( $has_refund && $key === 'order_total' ) {
					...
					if ( wc_tax_enabled() && 'incl' == $order->tax_display_cart ) {
						...
						if ( ! empty( $tax_del_array ) ) {
							$refunded_tax_del .= ' ' . sprintf( __( '(Includes %s)', 'youplay' ), implode( ', ', $tax_del_array ) );
						}

						if ( ! empty( $tax_ins_array ) ) {
							$refunded_tax_ins .= ' ' . sprintf( __( '(Includes %s)', 'youplay' ), implode( ', ', $tax_ins_array ) );
						}
					}
		...
		if ( $has_refund ) { ?>
			<tr>
				<th scope="row"><?php _e( 'Refunded:', 'youplay' ); ?></th>
		...
		if ( '' != $order->customer_note ) { ?>
			<tr>
				<th scope="row"><?php _e( 'Note:', 'youplay' ); ?></th>
...
<header>
	<h2><?php _e( 'Customer details', 'youplay' ); ?></h2>
</header>
<table class="table table-bordered">
<?php
