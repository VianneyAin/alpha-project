...
function yp_tabs($atts, $content = null) {
    ...
    foreach ( $tab_titles as $tab ) {
        ...
        if(isset($tab_atts['title'])) {
            $tabID = esc_attr('tab-' . ( isset( $tab_atts['tab_id'] ) ? $tab_atts['tab_id'] : sanitize_title( $tab_atts['title'] ) ));
            $tabs_nav .=
...
function yp_tab($atts, $content = null) {
    ...
    if(isset($title)) {
        $result .= '<div role="tabpanel" class="tab-pane ' . (yp_check($active) ? 'active' : '') . '" id="' . esc_attr('tab-' . ( isset( $tab_id ) ? $tab_id : $id )) . '">
...
function vc_youplay_tabs() {
    if(function_exists("wpb_map")) {
        ...
        wpb_map( array(
           "name"                    => __("YP Tabs", "youplay"),
           "base"                    => "yp_tabs",
           "category"                => "Youplay",
           "icon"                    => "icon-youplay",
           "show_settings_on_create" => false,
           "is_container"            => true,
           "admin_enqueue_js"        => get_template_directory_uri() . "/shortcodes/js/yp-tabs-vc-view.js",
           "admin_enqueue_css"       => get_template_directory_uri() . "/shortcodes/css/yp-tabs-vc-view.css",
           "params"                  => array(
              array(
                 "type"        => "checkbox",
           ...
           "default_content" => "
                [yp_tab title='" . __( 'Tab 1', 'youplay' ) . "' active='true'][/yp_tab]
                [yp_tab title='" . __( 'Tab 2', 'youplay' ) . "'][/yp_tab]",
           "js_view" => "YPTabsView"
        ) );
    }
}
